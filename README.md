...
> **G0.2 + ZCG-10 + ZCG-5 + ZCG-9 + ZCG-12 + ZCG-11** — hexagonal packages,
> `New` / `Close` / `Config` / stub `Zeus`+`Agent` handles, domain IDs, family
> `ErrorCode` catalogue, `security` DefaultRedactor, concurrent-safe
> `domain/journal`, `config` RuntimeConfig, and injected port interfaces
> (clock, ids, HTTP, Zeus, LLM, catalog, jobs, secrets). HTTP transport is
> ZCG-8. Claim remains **candidate** until a human
...
github.com/koten-ai/zeus_client_golang
  ...
  adapters/        # secretsenv (ZCG-12); zeushttp (ZCG-8), llmopenai, catalogfs, otlp, jobsfake later
